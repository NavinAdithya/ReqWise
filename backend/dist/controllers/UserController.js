...
class UserController {
    static async getQAPerformance(req, res) {
        try {
            if (!req.user || req.user.role !== 'ADMIN') {
                return res.status(401).json({ message: 'Unauthorized' });
            }
            const qas = await User_1.User.find({ role: 'QA' }).select('-password');
            const performanceData = await Promise.all(qas.map(async (qa) => {
                // 1. Report Quality
                const reports = await Report_1.Report.find({ qa: qa._id, status: { $ne: 'DRAFT' } });
                ...
                const mistakes = await Mistake_1.Mistake.find({ qa: qa._id });
                ...
                // 3. Assessment Score
                const assessments = await Assessment_1.Assessment.find({ qa: qa._id, status: 'COMPLETED' });
                ...
                // Active Reviews
                const activeReviews = await Requirement_1.Requirement.find({ assignedQA: qa._id, status: 'UNDER_ANALYSIS' }).select('title project category status');
                // Calculate Accuracy %
