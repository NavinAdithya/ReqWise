...
describe('REQWISE Full Backend Integration & Verification Tests', () => {
    ...
    beforeAll(async () => {
        ...
        await ReviewDecision_1.ReviewDecision.deleteMany({});
    ...
    describe('STEP 4: Database Model Validation', () => {
        test('Schemas contain all required fields and references', async () => {
            ...
            const checklist = new RequirementChecklist_1.RequirementChecklist({
                requirement: req._id,
                checklistVersion: 2,
            ...
            const decision = new ReviewDecision_1.ReviewDecision({
            ...
            const audit = new AuditLog_1.AuditLog({
                actor: new mongoose_1.default.Types.ObjectId(adminId),
                action: 'TEST_ACTION',
                entity: 'Requirement',
                entityId: req._id
            });
    ...
    describe('STEP 5: Role-Based Access Control (RBAC) Protections', () => {
        ...
        test('Admin can assign and review reports', async () => {
            ...
            const newReq = await Requirement_1.Requirement.create({
            ...
            await RequirementChecklist_1.RequirementChecklist.create({ requirement: newReq._id, checklistVersion: 2, items: [] });
            ...
            const updatedReq = await Requirement_1.Requirement.findById(newReq._id);
    ...
    describe('STEP 6: Requirement State Machine Transitions', () => {
        test('Transitions from DRAFT -> ASSIGNED -> UNDER_ANALYSIS -> REPORT_GENERATED -> UNDER_REVIEW -> CLIENT_REVIEW -> FINALIZED are valid', async () => {
            const req = await Requirement_1.Requirement.create({
            ...
            await RequirementChecklist_1.RequirementChecklist.create({ requirement: req._id, checklistVersion: 2, items: [] });
            ...
            const finalReq = await Requirement_1.Requirement.findById(req._id);
    ...
    describe('STEP 8: Comparative Validation Engine Verification', () => {
        test('Validation results verify checklist coverage and flags warnings without auto-generation', async () => {
            ...
            await Requirement_1.Requirement.create({
            ...
            const checklist = await RequirementChecklist_1.RequirementChecklist.create({
                requirement: req._id,
                checklistVersion: 2,
                items: [
    ...
    describe('STEP 9: Mistake Tracking & Assessment Engine triggers', () => {
        ...
        test('Case 1: Single mistake is ignored and does not trigger assessment', async () => {
            ...
            const assessments = await Assessment_1.Assessment.find({ qa: qaId });
        ...
        test('Case 3: Same mistake in 3-4 projects of same category triggers assessment when weight >= 10', async () => {
            const req1 = await Requirement_1.Requirement.create({ title: 'Req 1', description: 'D', client: new mongoose_1.default.Types.ObjectId(clientId), category: 'Fintech', project: 'P1' });
            const req2 = await Requirement_1.Requirement.create({ title: 'Req 2', description: 'D', client: new mongoose_1.default.Types.ObjectId(clientId), category: 'Fintech', project: 'P2' });
            const req3 = await Requirement_1.Requirement.create({ title: 'Req 3', description: 'D', client: new mongoose_1.default.Types.ObjectId(clientId), category: 'Fintech', project: 'P3' });
            const req4 = await Requirement_1.Requirement.create({ title: 'Req 4', description: 'D', client: new mongoose_1.default.Types.ObjectId(clientId), category: 'Fintech', project: 'P4' });
